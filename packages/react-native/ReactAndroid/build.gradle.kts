...
tasks.creating(Download::class) {
    ...
    src(
        "https://boostorg.jfrog.io/artifactory/main/release/${
            BOOST_VERSION.replace(
                "_",
                "."
            )
        }/source/boost_${BOOST_VERSION}.tar.gz"
    )
    onlyIfModified(true)
    overwrite(false)
    retries(5)
...
android {
    ...
    prefab {
        ...
        create("runtimeexecutor") {
            headers = File(prefabHeadersDir, "runtimeexecutor").absolutePath
        }
        ...
        create("hermes_executor") {
            headers = File(prefabHeadersDir, "hermes_executor").absolutePath
        }
