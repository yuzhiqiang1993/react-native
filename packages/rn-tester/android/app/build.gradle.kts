...
println("Using CMake version $cmakeVersion")
...
//  return value?.toString()?.split(",") ?: listOf("armeabi-v7a", "x86", "x86_64", "arm64-v8a")
    return value?.toString()?.split(",") ?: listOf("arm64-v8a")//只编译arm64-v8a
}
...
afterEvaluate {
    ...
    if (isNewArchEnabled) {
        ...
        tasks.getByName("mergeHermesReleaseNativeLibs")
            .mustRunAfter("externalNativeBuildJscRelease")
        tasks.getByName("mergeJscDebugNativeLibs").mustRunAfter("externalNativeBuildHermesDebug")
        tasks.getByName("mergeJscReleaseNativeLibs")
            .mustRunAfter("externalNativeBuildHermesRelease")
    }
